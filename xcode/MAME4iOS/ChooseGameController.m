...
#define ALL_SCOPES          @[@"系统", @"软件", @"复刻", @"厂商", @"年份", @"体裁", @"驱动"]
...
- (void)filterGameList
{
    ...
    for (NSString* section in @[kGameInfoTypeSoftware, kGameInfoTypeConsole, kGameInfoTypeComputer, kGameInfoTypeBIOS, kGameInfoTypeSnapshot, @"未知"]) {
...
- (NSArray*)menuActionsForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (game.gameIsSnapshot) {
        return @[
            [UIAlertAction actionWithTitle:@"用作封面" symbol:@"photo" style:UIAlertActionStyleDefault handler:^(id action) {
    ...
    if ([self getSystemsForGame:game].count > 1) {
        actions = [actions arrayByAddingObject:
            [UIAlertAction actionWithTitle:@"一起玩..." symbol:@"ellipsis.circle" style:UIAlertActionStyleDefault handler:^(id action) {
    ...
    actions = [actions arrayByAddingObjectsFromArray:@[
        [UIAlertAction actionWithTitle:@"详情" symbol:@"info.circle" style:UIAlertActionStyleDefault handler:^(id action) {
            [self info:game];
        }]
    ]];
    ...
    if (@available(iOS 12.0, *)) {
        ...
        if (activity != nil) {
            actions = [actions arrayByAddingObjectsFromArray:@[
                [UIAlertAction actionWithTitle:@"添加到Siri" symbol:(shortcut ? @"checkmark.circle" : @"plus.circle") style:UIAlertActionStyleDefault handler:^(id action) {
