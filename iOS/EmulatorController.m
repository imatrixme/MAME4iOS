...
-(void)runMenu:(id)sender {
    ...
    if(myosd_inGame && myosd_in_menu==0)
    {
        ...
        if (gamepad == nil) {
            // CONFIGURE and PAUSE
            [menu addButtons:@[@":slider.horizontal.3:配置",@":pause.circle:暂停"] handler:^(NSUInteger button) {
                if (button == 0)
                    push_mame_key(MYOSD_KEY_CONFIGURE);
                else
                    push_mame_key(MYOSD_KEY_P);
        ...
        if (put_keyboard_on_menu) {
            ...
            [menu addButtons:@[kb, @":wrench:服务"] handler:^(NSUInteger button) {
                if (button == 0)
            ...
            [menu addButtons:@[@":camera:快照", @":wrench:服务"] handler:^(NSUInteger button) {
                if (button == 0)
        ...
        // Power and Reset
        [menu addButtons:@[@":power:电源", @":escape:重置"] handler:^(NSUInteger button) {
            if (button == 0)
    ...
    [menu addButton:@":gear:设置" handler:^{
        [self runSettings];
    }];
    [menu addButton:(myosd_inGame && myosd_in_menu==0) ? @"退出游戏" : @"退出" style:UIAlertActionStyleDestructive handler:^{
        [self runExit:NO];
    }];

    // Cancel button wont show (cuz we are an action sheet) but we need it for auto dismiss
    [menu addButton:@"取消" style:UIAlertActionStyleCancel handler:^{}];
...
-(void)buildHUD {
    ...
    if (g_pref_showHUD == HudSizeLarge) {
        ...
        [hudViewController addButtons:@[@":bookmark:加载 ①", @":bookmark:加载 ②"] handler:^(NSUInteger button) {
            mame_load_state((int)button + 1);
        }];
        [hudViewController addButtons:@[@":bookmark.fill:保存 ①", @":bookmark.fill:保存 ②"] handler:^(NSUInteger button) {
            mame_save_state((int)button + 1);
        }];
        [hudViewController addButtons:@[@":slider.horizontal.3:配置",@":pause.circle:暂停"] handler:^(NSUInteger button) {
            if (button == 0)
                push_mame_key(MYOSD_KEY_CONFIGURE);
            else
                push_mame_key(MYOSD_KEY_P);
        }];
        [hudViewController addButtons:@[@":camera:快照", @":wrench:服务"] handler:^(NSUInteger button) {
            if (button == 0)
                push_mame_key(MYOSD_KEY_SNAP);
            else
                push_mame_key(MYOSD_KEY_SERVICE);
        }];
        [hudViewController addButtons:@[@":power:电源", @":escape:重置"] handler:^(NSUInteger button) {
            if (button == 0)
                push_mame_key(MYOSD_KEY_RESET);         // this does a HARD reset
            else
                push_mame_key(MYOSD_KEY_F3);            // this does a SOFT reset
        }];
        [hudViewController addButton:(myosd_inGame && myosd_in_menu==0) ? @":xmark.circle:退出游戏" : @":xmark.circle:退出" color:[UIColor.systemRedColor colorWithAlphaComponent:0.5] handler:^{
...
- (void)webServerShowAlert:(GCDWebServer*)server {
    ...
    NSString* message = [NSString stringWithFormat:@"要想将 ROM 从电脑上传送到此设备，请在电脑 web 浏览器中打开以下网址::\n\n%@",servers];
...
- (void)runBenchmark
{
    ...
    [self showAlertWithTitle:title message:msg buttons:@[@"停止"] handler:^(NSUInteger button) {
